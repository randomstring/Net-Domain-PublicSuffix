...
always try to return a reasonable answer. public_suffix return an
...
Returns true id the domain of the provided string exists in the list
of valid top level domains. The list of valid domains is
...
public_suffix is the same as base_domain run in strict
mode.
...
There are some rules that Basedomain know about thant
PublicSuffix. This is due to additional research. For instance
http://en.wikipedia.org/wiki/.mt lists gov.mt as a valid TLD, but it
is missing from the publicsuffix.org list.
...
Basedomain does not honor the "!" exception rules because there are
very few of them (19 at current count) and the impact on real word is
