...
#
# Use an older mozilla TLD config file as a test of our basedomain function.
#
# Latest versions available from:
#
#  https://publicsuffix.org/list/effective_tld_names.dat
#
# previous: http://lxr.mozilla.org/mozilla/source/netwerk/dns/src/effective_tld_names.dat?raw=1
#
...
#    next if ($line =~ /^\!/);
#    next if ($line =~ /[^a-z-A-Z0-9\.\-]/);
#    next if ($basedomain =~ /\.us$/);   # most .us domains follow a more complicated pattern
    ...
    if ($basedomain =~ s/^\!//)
    {
        ...
        # <toplevel>   => <toplevel>
        $hostname = $basedomain;
        my $bn = base_domain($hostname);
        is($bn, $basedomain, "!$basedomain <tld> -> <tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");

        # www.foobar.<toplevel>   => <toplevel>
        $hostname = "www.foobar.$basedomain";
        $bn = base_domain($hostname);
        is($bn, $basedomain, "!$basedomain www.foobar.<tld> -> <tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");

        # foobar.<toplevel>   => <toplevel>
        $hostname = "foobar.$basedomain";
        $bn = Net::Domain::PublicSuffix::base_domain($hostname);
        is($bn, $basedomain, "!$basedomain foobar.<tld> -> <tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");

        # f.foobar.<toplevel>   => <toplevel>
        $hostname = "f.foobar.$basedomain";
        $bn = Net::Domain::PublicSuffix::base_domain($hostname);
        is($bn, $basedomain, "!$basedomain f.foobar.<tld> -> <tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");
        ...
        # www.foobar.<toplevel>   => foobar.<toplevel>
        my $bn = base_domain($hostname);
        is($bn, $basedomain, "www.foobar.<tld> -> foobar.<tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");

        # foobar.<toplevel>   => foobar.<toplevel>
        $hostname = $basedomain;
        $bn = Net::Domain::PublicSuffix::base_domain($hostname);
        is($bn, $basedomain, "foobar.<tld> -> foobar.<tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");

        # f.foobar.<toplevel>   => foobar.<toplevel>
        $basedomain = "f.$save_basedomain";
        $hostname = $basedomain;
        $bn = Net::Domain::PublicSuffix::base_domain($hostname);
        is($bn, $basedomain, "f.foobar.<tld> -> foobar.<tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");

        # w.w.<toplevel>   => w.<toplevel>
        $basedomain = "w.$save_basedomain";
        if ($basedomain !~ /^w\.(er|se|bg)$/) {  # w.bg, w.se and w.er are valid toplevels
            $hostname = "w.$basedomain";
            $bn = Net::Domain::PublicSuffix::base_domain($hostname);
            is($bn, $basedomain, "w.w.<tld> -> w.<tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");
        }

        # w.m.<toplevel>   => m.<toplevel>
        $basedomain = "m.$save_basedomain";
        if ($basedomain !~ /^m\.(?:se|bg)$/) {   # m.se and m.bg is a valid toplevel
            $hostname = "w.$basedomain";
            $bn = Net::Domain::PublicSuffix::base_domain($hostname);
            is($bn, $basedomain, "w.m.<tld> -> m.<tld>  [$hostname] -> [$bn]  EXPECT [$basedomain]");
        }
    }
}
exit(0);
